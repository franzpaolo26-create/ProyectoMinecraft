...
async function rcon(command) {
  const { stdout, stderr } = await execAsync(
    `mcrcon -H ${RCON_HOST} -P ${RCON_PORT} -p ${RCON_PASS} "${command.replace(/"/g, '\\"')}"`
  );
...
router.get('/server/status', async (req, res) => {
  try {
    ...
    const online = mc.pm2_env.status === 'online';
    res.json({
      online,
...
router.post('/server/spawn', async (req, res) => {
  ...
  try {
    // Summon en las coordenadas del jugador usando el comando summon con ~ ~ ~
    // Primero ejecutamos en el contexto del jugador
    const result = await rcon(`execute at ${player} run summon ${mob} ~ ~ ~`);
    // Si amount > 1, repetimos
    for (let i = 1; i < Math.min(parseInt(amount), 20); i++) {
      await rcon(`execute at ${player} run summon ${mob} ~ ~ ~`);
    }
    res.json({ message: result || `Spawneado ${amount}x ${mob} en ${player}.` });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
